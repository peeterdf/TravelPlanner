    ...
    <div className="p-4 space-y-4 pb-28">
      ...
      {Object.keys(byPerson).length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
          <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-3">Resumen por persona</p>
          <div className="space-y-2">
            {Object.entries(byPerson).sort((a, b) => b[1] - a[1]).map(([person, total]) => {
              const diff = total - perPerson
              return (
                <div key={person} className="flex items-center justify-between">
                  <span className="font-medium text-gray-900 text-sm">{person}</span>
                  <div className="text-right">
                    <span className="font-bold text-gray-900 text-sm">{total.toFixed(0)}</span>
                    <span className={`ml-2 text-xs font-medium ${diff > 0.5 ? 'text-green-600' : diff < -0.5 ? 'text-red-600' : 'text-gray-400'}`}>
                      {diff > 0.5 ? `le deben ${diff.toFixed(0)}` : diff < -0.5 ? `debe ${Math.abs(diff).toFixed(0)}` : '✓ par'}
                    </span>
                  </div>
                </div>
              )
          ...
          <div className="border-t border-gray-100 mt-3 pt-3 flex justify-between text-sm">
            ...
            <span className="font-bold text-gray-900">{grandTotal.toFixed(0)}</span>
          </div>
          <div className="flex justify-between text-xs text-gray-500 mt-1">
            <span>Por persona ({Object.keys(byPerson).length} viajeros)</span>
            <span className="font-medium">{perPerson.toFixed(0)}</span>
          </div>
        </div>
      )}
      ...
      {trip.expenseSplits.length === 0 ? (
        ...
        <div className="space-y-2">
          {trip.expenseSplits.map(s => (
            <div key={s.id} className="bg-white rounded-xl border border-gray-100 shadow-sm px-4 py-3 flex items-center justify-between gap-2">
              <div className="flex-1 min-w-0">
                <p className="font-medium text-gray-900 text-sm truncate">{s.concept}</p>
                <p className="text-xs text-gray-500 mt-0.5">
                  Pagó: <span className="font-semibold text-gray-700">{s.paidBy}</span>
                  {' · '}<span className="font-medium">{s.currency} {s.amount.toFixed(0)}</span>
                </p>
              </div>
              <div className="flex gap-1 shrink-0">
                <button onClick={() => openEdit(s)} className="p-1.5 text-gray-400 hover:text-gray-700"><Pencil size={15} /></button>
        ...
        <Modal title={modal === 'add' ? 'Nuevo registro' : 'Editar registro'} onClose={() => setModal(null)}>
          <div className="space-y-3">
            <div>
              ...
              <input className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm" placeholder="Alojamiento Londres"
                value={form.concept} onChange={e => f('concept', e.target.value)} autoFocus />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Pagado por *</label>
              {travelerNames.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {travelerNames.map(n => (
                    <button key={n} onClick={() => f('paidBy', n)}
                      className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${form.paidBy === n ? 'bg-blue-600 text-white border-blue-600' : 'border-gray-300 text-gray-600'}`}>
                      {n}
                    </button>
                  ))}
                </div>
              ) : (
                <input className="w-full border border-gray-300 rounded-xl px-3 py-2 text-sm" placeholder="Nombre"
