  ...
  return (
    <div className="p-4 space-y-3 pb-28">
      {sorted.length === 0 ? (
        ...
        sorted.map(a => (
          <div key={a.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
            <div className="p-4">
            ...
            <div className="flex border-t border-gray-100 dark:border-gray-700">
              {a.url && (
                <a href={a.url} target="_blank" rel="noopener noreferrer"
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs text-blue-500 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                  aria-label="Abrir reserva">
                  <ExternalLink size={14} /><span>Reserva</span>
                </a>
              )}
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                title="Agregar a gastos"
              >
                <CircleDollarSign size={14} /><span>Gasto</span>
              </button>
              <button
                onClick={() => openEdit(a)}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
              ...
              <button
                onClick={() => { if (confirm(`¿Eliminar alojamiento en ${a.city}?`)) deleteAccommodation(tripId!, a.id) }}
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
              >
